...
if __name__ == "__main__":
    ...
    for num,seq in enumerate(seq_list):
        ...
        if os.path.exists(os.path.join('./new-networkv2-146-results/' + 'new-network-v2-146' + '_' + seq + '.txt')):
            os.remove(os.path.join('./new-networkv2-146-results/' + 'new-network-v2-146' + '_' + seq + '.txt'))
        for loc in res['res']:
            loc_8.append(loc[0])
            loc_8.append(loc[1])
            ...
            loc_8.append(loc[0])
            loc_8.append(loc[1] + loc[3])
            with open(os.path.join('./new-networkv2-146-results/' + 'new-network-v2-146'+'_' + seq + '.txt'), 'a') as f:
                count = 0
                for k in loc_8:
                    count +=1
                    f.write(str(k))
