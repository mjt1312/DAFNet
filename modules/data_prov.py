...
class RegionDataset(data.Dataset):
    def __init__(self, img_dir, img_list_v, img_list_i, gt_v, gt_i, receptive_field, opts):
        if pretrain_opts['set_type'] =='RGB-T234' or pretrain_opts['set_type'] =='VOT-RGBT':
            self.img_list_v= np.array([os.path.join(img_dir, 'visible', img) for img in img_list_v])
            self.img_list_i= np.array([os.path.join(img_dir, 'infrared', img) for img in img_list_i])
            self.gt_v= gt_v
            self.gt_i= gt_i
        ...
        if pretrain_opts['set_type'] =='50':
            self.img_list_v= np.array([os.path.join(img_dir, 'v', img) for img in img_list_v])
            self.img_list_i= np.array([os.path.join(img_dir, 'i', img) for img in img_list_i])
            self.gt_v= gt_v
            self.gt_i= gt_i
            image_v = Image.open(self.img_list_v[0]).convert('RGB')
            image_i = Image.open(self.img_list_i[0]).convert('RGB')
    ...
    def __next__(self):
        ...
        scenes_v = []
        scenes_i = []
        for i, (img_path_v, bbox_v, img_path_i, bbox_i) in enumerate(zip(self.img_list_v[idx], self.gt_v[idx], self.img_list_i[idx], self.gt_i[idx])):
            ...
            pos_examples = gen_samples(SampleGenerator('gaussian', (ishape[1],ishape[0]), 0.1, 1.2, 1.1, False), bbox_v, n_pos, overlap_range=self.overlap_pos)
            neg_examples = gen_samples(SampleGenerator('uniform', (ishape[1],ishape[0]), 1, 1.2, 1.1, False), bbox_v, n_neg, overlap_range=self.overlap_neg)

            if pos_examples.shape[0] == 0:
                pos_examples = bbox_v.reshape(1,4)
                for e in range(63):
                    pos_examples = np.concatenate((pos_examples, bbox_v.reshape(1,4)), axis = 0)
            ...
            crop_img_size = (padded_scene_box[2:4] * ((pretrain_opts['img_size'], pretrain_opts['img_size']) / bbox_v[2:4])).astype('int64') * jitter_scale[0][0]
            ...
            pos_rois = samples2maskroi(pos_rois, self.receptive_field, (jittered_obj_size, jittered_obj_size),bbox_v[2:4], pretrain_opts['padding'])
            ...
            neg_rois = samples2maskroi(neg_rois, self.receptive_field, (jittered_obj_size, jittered_obj_size),bbox_v[2:4], pretrain_opts['padding'])
