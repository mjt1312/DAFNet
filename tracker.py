...
def run_mdnet(img_list_v,img_list_t, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    result = np.zeros((len(img_list_t),4))
    result_bb = np.zeros((len(img_list_t),4))
    result[0] = np.copy(target_bbox)
    result_bb[0] = np.copy(target_bbox)

    iou_result = np.zeros((len(img_list_t),1))

    # execution time array
    exec_time_result = np.zeros((len(img_list_t),1))

    # Init model
    model = MDNet(opts['model_path'],opts['model_path2'],opts['model_path4'],opts['model_path5'],opts['model_path_sk_fc1'],
                  opts['model_path_sk_fcs1'],opts['model_path_sk_fc2'],opts['model_path_sk_fcs2'],opts['model_path_sk_fc3'],opts['model_path_sk_fcs3'])
    ...
    cur_image_t = Image.open(img_list_t[0]).convert('RGB')
    cur_image_t = np.asarray(cur_image_t)
    # Draw pos/neg samples
    ishape = cur_image_t.shape
    ...
    for bidx in range(0,scene_boxes.shape[0]):
        ...
        cropped_image_t, cur_image_var_t = img_crop_model.crop_image(cur_image_t, np.reshape(scene_boxes[bidx],(1,4)), crop_img_size)
        cropped_image_t = cropped_image_t- 128.
        feat_map = model(cropped_image_v,cropped_image_t, out_layer='conv3')
    ...
    for iidx in range(replicateNum):
        ...
        cur_extra_cropped_image_t, _ = img_crop_model.crop_image(cur_image_t, np.reshape(extra_scene_box,(1,4)),extra_crop_img_size)
        cur_extra_cropped_image_t = cur_extra_cropped_image_t.detach()
        ...
        if iidx==0:
            ...
            extra_cropped_image_t = cur_extra_cropped_image_t
            ...
            extra_cropped_image_t = torch.cat((extra_cropped_image_t,cur_extra_cropped_image_t),dim=0)
    ...
    extra_cropped_image_t -= 128.

    extra_feat_maps = model(extra_cropped_image_v, extra_cropped_image_t,out_layer='conv3')
    # Draw pos/neg samples
    ishape = cur_image_t.shape
    ...
    if display or savefig:
        dpi = 80.0
        figsize = (cur_image_t.shape[1]/dpi, cur_image_t.shape[0]/dpi)

        fig = plt.figure(frameon=False, figsize=figsize, dpi=dpi)
        ax = plt.Axes(fig, [0., 0., 1., 1.])
        ax.set_axis_off()
        fig.add_axes(ax)
        im = ax.imshow(cur_image_t)
    ...
    for i in range(1,len(img_list_t)):

        tic = time.time()
        # Load image
        cur_image_v = Image.open(img_list_v[i]).convert('RGB')
        cur_image_v = np.asarray(cur_image_v)
        cur_image_t = Image.open(img_list_t[i]).convert('RGB')
        cur_image_t = np.asarray(cur_image_t)
        #cur_image=np.concatenate((cur_image_v,cur_image_t),axis=2)
        ...
        if padded_scene_box[0] > cur_image_t.shape[1]:
            padded_scene_box[0] = cur_image_t.shape[1]-1
        if padded_scene_box[1] > cur_image_t.shape[0]:
            padded_scene_box[1] = cur_image_t.shape[0]-1
        ...
        cropped_image_v = cropped_image_v - 128.
        cropped_image_t,cur_image_var_t = img_crop_model.crop_image(cur_image_t, np.reshape(padded_scene_box,(1,4)),crop_img_size)
        cropped_image_t = cropped_image_t - 128.
        model.eval()
        feat_map = model(cropped_image_v,cropped_image_t, out_layer='conv3')
        ...
        if opts['visual_log']:
            if gt is None:
                print "Frame %d/%d, Score %.3f, Time %.3f" % \
                    (i, len(img_list_t), target_score, spf)
            else:
                print "Frame %d/%d, Overlap %.3f, Score %.3f, Time %.3f" % \
                    (i, len(img_list_t), overlap_ratio(gt[i],result_bb[i])[0], target_score, spf)
        iou_result[i]= overlap_ratio(gt[i],result_bb[i])[0]


    fps = len(img_list_t) / spf_total
    #fps = (len(img_list_t)-1) / spf_total #no first frame
